...
//		Border(posBorder5, 0, 0, 8, height);
	...
	void	TestAssignResrc(float rnd, int tileX, int tileY) {
		...
		if (rnd < 0.15f)
		{
			...
			{
				variant_ressources[tileY, tileX] = GameObject.Instantiate(ressources_obj, vec + tiles[tileY, tileX].transform.position, ressources_obj.transform.rotation) as GameObject;
				variant_ressources[tileY, tileX].transform.parent = tiles[tileY, tileX].transform;
	...
	void	Generate(int h, int w) {
		...
		for (int z = 0; z < height; z++)
		{
			for (int x = 0; x < width; x++)
			{
				...
				vec.z = j * tile.transform.localScale.z * 10;
				tiles[z, x] = GameObject.Instantiate(tile, vec, tile.transform.rotation) as GameObject;
				tiles[z, x].transform.parent = lines[j].transform;
				tiles[z, x].transform.name = "Tiles(" + z + ", " + x + ")";

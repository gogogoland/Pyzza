...
char	*command_write_msg(t_client *clt, int action, int n, char *pbc)
{
	...
	if (action < 6 && asprintf(&msg, g_msg_cmd[action], clt->name) >= 0)
		return (msg);
	else if (action == 6 && asprintf(&msg, g_msg_cmd[action], clt->name,
...
char	*command_write_msg_end_fork(t_client *clt, t_client *frk)
{
	...
	asprintf(&msg, g_msg_cmd[13], clt->name, frk->name);
	return (msg);
}
